...
```sh
cd ~
curl -fLO https://github.com/Harmonybrew/ohos-ruby/releases/download/4.0.5/ruby-4.0.5-ohos-arm64.tar.gz
tar -zxf ruby-4.0.5-ohos-arm64.tar.gz
export PATH=~/ruby-4.0.5-ohos-arm64/bin:$PATH
...
示例：
```sh
hdc file send ruby-4.0.5-ohos-arm64.tar.gz /data
hdc shell

cd /data
tar -zxf ruby-4.0.5-ohos-arm64.tar.gz
export PATH=/data/ruby-4.0.5-ohos-arm64/bin:$PATH
...
curl -fLO https://github.com/Harmonybrew/ohos-ruby/releases/download/4.0.5/ruby-4.0.5-ohos-arm64.tar.gz
tar -zxf ruby-4.0.5-ohos-arm64.tar.gz
export PATH=/opt/ruby-4.0.5-ohos-arm64/bin:$PATH
...
需要在 [鸿蒙容器](https://github.com/hqzing/dockerharmony) 中运行项目里的 build.sh，以实现 ruby 的本地编译。
