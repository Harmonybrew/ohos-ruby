...
./Configure --prefix=/opt/openssl-3.6.1-ohos-arm64 --openssldir=/etc/ssl no-legacy no-module no-shared no-engine linux-aarch64
make -j$(nproc)
make install_dev
cd ..

# 编译 yaml
curl -fSLO https://github.com/yaml/libyaml/releases/download/0.2.5/yaml-0.2.5.tar.gz
...
cd libffi-3.5.2
./configure --prefix=/opt/libffi-3.5.2-ohos-arm64 --enable-static --disable-shared --disable-docs --host=aarch64-linux
make -j$(nproc)
make install
cd ..
...
curl -fSLO https://cache.ruby-lang.org/pub/ruby/4.0/ruby-4.0.1.tar.gz
...
./configure \
  ...
  --with-opt-dir=/opt/openssl-3.6.1-ohos-arm64:/opt/yaml-0.2.5-ohos-arm64:/opt/zlib-1.3.1-ohos-arm64:/opt/libffi-3.5.2-ohos-arm64 \
