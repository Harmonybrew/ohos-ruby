...
curl -fLO https://cache.ruby-lang.org/pub/ruby/4.0/ruby-4.0.7.tar.gz
tar -zxf ruby-4.0.7.tar.gz
cd ruby-4.0.7
patch -p1 < ../0001-add-target-os.patch
autoconf
./configure \
  --prefix=/opt/ruby-4.0.7-ohos-arm64 \
  --host=aarch64-linux \
...
cat <<EOF > /opt/ruby-4.0.7-ohos-arm64/licenses.txt
...
# 打包最终产物
cp -r /opt/ruby-4.0.7-ohos-arm64 ./
tar -zcf ruby-4.0.7-ohos-arm64.tar.gz ruby-4.0.7-ohos-arm64
